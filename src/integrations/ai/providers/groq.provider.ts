...
export class GroqProvider implements AIProvider {
  ...
  constructor(
    ...
    if (!apiKey) {
      throw new Error(
        'GROQ_API_KEY is required when AI_PROVIDER=groq. Get a free key at https://console.groq.com/keys'
      );
    }
    ...
    try {
      ...
      try {
        ...
        throw new AIProviderError(
          `Groq returned a response that didn't match the expected schema for ${operation}`,
          this.name,
          operation,
          false,
          parseErr
      ...
      throw new AIProviderError(
        `Groq request failed for ${operation}: ${message}`,
        this.name,
        operation,
